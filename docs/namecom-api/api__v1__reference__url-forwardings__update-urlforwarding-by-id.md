...
````yaml patch /core/v1/urlforwarding/{domainName}/{id}
openapi: 3.1.0
info:
  contact:
...
paths:
  /core/v1/urlforwarding/{domainName}/{id}:
    patch:
      ...
      description: >-
        Modifies an existing URL forwarding rule by ID.  The domain must be
        owned by the authenticated account. Changes may take up to 24 hours to
        fully propagate.
      operationId: UpdateURLForwardingById
      parameters:
        - description: >-
            DomainName is the domain that owns the URL forwarding entry. Must be
          ...
          schema:
            ...
            format: hostname
        - description: >-
            ID is the server-assigned unique identifier for the URL forwarding
            record (returned in list responses).
          in: path
          name: id
          required: true
          schema:
      ...
      responses:
        ...
        '404':
          description: >-
            Domain or URL forwarding entry not found, or domain not owned by the
...
components:
  schemas:
    URLForwardingInput:
      description: 'Fields for creating or updating a URL forwarding entry. '
      type: object
      required:
        - host
        - forwardsTo
