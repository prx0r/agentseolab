...
````yaml get /core/v1/urlforwarding/{domainName}
openapi: 3.1.0
info:
  contact:
...
paths:
  /core/v1/urlforwarding/{domainName}:
    get:
      ...
      description: >-
        Returns all URL forwarding settings configured for a domain. Each entry
        includes an `id` that can be used with the URL Forwarding by-ID
        endpoints to get, update, or delete records.
      operationId: ListURLForwardingsByDomain
      parameters:
        - description: >-
            DomainName is the domain to list URL forwarding entries for. The
            domain must be owned by the authenticated account.
          in: path
          name: domainName
          required: true
          schema:
