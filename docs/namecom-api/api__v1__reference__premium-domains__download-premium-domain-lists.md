...
````yaml get /core/v1/premiumdomainslist
openapi: 3.1.0
info:
  contact:
...
paths:
  /core/v1/premiumdomainslist:
    get:
      ...
      description: >-
        Gets a pre-signed URL that will allow a user to download a list of
        premium domains, with their registration and renewal pricing.

        **Please Note:** The pre-signed URL will only be valid for 10 minutes.
        This endpoint is only available to approved reseller accounts. Contact
        name.com support to request access.
      operationId: PremiumDomainLists
      responses:
        '200':
          description: >-
            A successful response containing the pre-signed URL to download the
            file.
          content:
            application/json:
              schema:
